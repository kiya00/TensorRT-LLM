...
class ThunderInferenceOptimizer1:
    ...
    def __call__(self, cm: CachedSequenceInterface) -> GraphModule:
        ...
        cm.info.set_example_sequence()
        # uses the config from egm, e.g. num_key_value_heads, head_dim, num_attention_heads to initialize the cache
        egm = torch_export_to_gm(model, args=cm.args, dynamic_shapes=cm.dynamic_shapes)
        ...
        src_attn_nodes = cmanager.get_attn_nodes(egm, torch.ops.attention.scaled_dot_product_attention.default)
        # Note that the egm is only used to get the config and initialize the cache, no transform is done on the egm
        cmanager.update_cache_initializers(src_attn_nodes)

        cm.initialize_caches()
        print(len(cm.args))
        print(model.config.num_key_value_heads, model.config.head_dim,model.config.num_attention_heads,"+++++++++++ config")

        # replace the attn op with the cached attn op
